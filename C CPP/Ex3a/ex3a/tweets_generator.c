...
int fill_database(FILE *fp, int words_to_read, MarkovChain *markov_chain)
{
         ...
         > markov_chain->linked_list->size))
  {
    ...
    > markov_chain->linked_list->size))
    {
      ...
      Node *node = add_to_database (markov_chain, copy_word);
      ...
      if (prev_node && !is_last (prev_node->data->data)) //no prev word
      {
        if (!add_node_to_frequencies_list (prev_node->data, node->data))
          // checking allocation
        {
          free_database(markov_ptr);
          return 1; //failed allocation.
...
FILE *file_opening(char *file_arg)
{
...
MarkovChain *allocate_chain()
{
  ...
  markov_chain->linked_list = linked_list;
...
int main (int argc, char *argv[])
{
  if (!is_valid_arg (argc))
  {
    ...
    {
      int words_to_read = strtol (argv[4], NULL, BASE);
      filling_database = fill_database (file, words_to_read, markov_chain);
    }
    if (filling_database)
      {
        free_database (p_chain);
